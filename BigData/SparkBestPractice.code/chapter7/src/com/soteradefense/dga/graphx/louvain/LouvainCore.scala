...
object LouvainCore {
   ...
   def createLouvainGraph[VD: ClassTag](graph: Graph[VD,Long]) : Graph[VertexState,Long]= {
    ...
    val nodeWeights = graph.aggregateMessages(nodeWeightMapFuncEx, nodeWeightReduceFuncEx)
    ...
    val louvainGraph = graph.outerJoinVertices(nodeWeights)((vid,data,weightOption)=> { 
      ...
      state.community = vid
      state.changed = false
      state.communitySigmaTot = weight._1 //+weight._2  计算入度
      state.internalWeight = 0L
      state.nodeWeight = weight._1+weight._2
      state
  ...
  def louvain(sc:SparkContext, graph:Graph[VertexState,Long], minProgress:Int=1,progressCounter:Int=1) : (Double,Graph[VertexState,Long],Int)= {
    ...
    var msgRDD = louvainGraph.aggregateMessages(sendMsgEx, mergeMsg)
  ...
  private def sendMsgEx(ctx:EdgeContext[VertexState, Long, Map[(Long,Long), Long]]):Unit = {
    // 计算一个节点和其所有出度节点的党派值
    //ctx.sendToDst(Map((ctx.srcAttr.community,ctx.srcAttr.communitySigmaTot)->ctx.attr))
    ctx.sendToSrc(Map((ctx.dstAttr.community,ctx.dstAttr.communitySigmaTot)->ctx.attr))
  }
  ...
  def compressGraph(graph:Graph[VertexState,Long],debug:Boolean=true) : Graph[VertexState,Long] = {
    ...
    val nodeWeightMapFuncEx = (ctx:EdgeContext[VertexState, Long, (Long,Long)]) => {
      ctx.sendToDst((ctx.attr,0L))
      ctx.sendToSrc((0L,ctx.attr))
    }
    val nodeWeightReduceFunc = (e1:Long,e2:Long) => e1+e2
    ...
    val nodeWeights = compressedGraph.aggregateMessages(nodeWeightMapFuncEx, nodeWeightReduceFuncEx)
   ...
   val louvainGraph = compressedGraph.outerJoinVertices(nodeWeights)((vid,data,weightOption)=> { 
      val weight = weightOption.getOrElse((0L,0L))
      //data.communitySigmaTot = weight._1+weight._2 +data.internalWeight
      data.communitySigmaTot = weight._1 + data.internalWeight // 计算入度
      data.nodeWeight = weight._1+weight._2
      data
    }).cache()
